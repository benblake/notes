...
- This command creates container from the `hello-world` images and runs it.
```
Hello from Docker!
...
Create a `Dockerfile` with configuration of how the container should behave. This Dockerfile then gets used by the Docker client and Docker server to create a conainer image. To create a Dockerfile you generally:
...
The command used in this Dockerfile are:
...
When you build run a container only the files specified in that container's image are present. You need some way to get your project files into the container to be executed.
...
## Node/Redis webapp example
This is the visits app. We are going to create a container running a node webapp and a separate container to run Redis. We will then see how to configure the web container to talk to the Redis container to store the results of number of visits to the site.

The Node webapp is pretty basic:
```javascript
const express = require('express');
const redis = require('redis');

const app = express();
const client = redis.createClient() // need configuration here;
client.set('visits', 0);
...
CMD ["npm", "start"]
```
The container running the webapp here will not (by default) be able to communicate with the container running Redis. To coordinate the two containers we will use Docker compose
...
To use Docker compose we will create a YAML file specifying the command we want to run. This file will get passed to the Docker compose CLI which will parse it and be responsible for running all the Docker commands we need. The Docker compose file for running our 2 containers is
```yml
version: '3'
services:
  redis-server:
  ...
  node-app:
    ...
    ports:
      - "4001:8081"
```
Note that Docker compose will run these containers so that they have access to each other - you do not need to set up any other networking infrastructure. We can now connect to the Redis instance running on the `redis-server` container nby changing the node app's client configuration to
```javascript
